  ...
  return (
    <>
      <PageHero
        ...
        image={galleryGroups[1].images[0].src}
      />
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-6 lg:py-24">
        ...
        <div className="space-y-16">
          {galleryGroups.map((group, groupIndex) => (
            <div key={group.title}>
              <Reveal>
                <div className="mb-6 flex flex-col justify-between gap-3 border-t-2 border-secondary pt-4 md:flex-row md:items-end">
                  <div>
                    <p className="text-xs font-extrabold text-accent">0{groupIndex + 1}</p>
                    <h3 className="mt-3 font-display text-2xl tracking-[-0.035em] text-primary sm:text-3xl">
                      {group.title}
                    </h3>
                  </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {group.images.map((item, imageIndex) => (
                  <Reveal key={item.label} delay={imageIndex * 80}>
                    <figure className="group overflow-hidden border border-border bg-background">
